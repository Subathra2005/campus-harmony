...
type HostelCsvKey = 'roomNumber' | 'block' | 'floor' | 'capacity' | 'type' | 'status' | 'monthlyRent';

const CSV_COLUMNS: Array<{ key: HostelCsvKey; label: string }> = [
...
const SAMPLE_ROW = 'B-203,Block B,2,3,double,available,4500';
const EXPECTED_COLUMNS_TEXT = CSV_COLUMNS.map(column => column.label).join(', ');
const VALID_TYPES = new Set(['single', 'double', 'triple']);
const VALID_STATUS = new Set(['available', 'occupied', 'maintenance']);

interface CsvRow {
  rowNumber: number;
  values: Record<HostelCsvKey, string>;
}
...
const parseCsv = (content: string): CsvRow[] => {
  ...
  const duplicateCheck = new Set<HostelCsvKey>();
  ...
  return lines.slice(1).map((line, index) => {
    ...
    const values = {} as Record<HostelCsvKey, string>;
...
const buildHostelRoom = (row: CsvRow): HostelRoom => {
  ...
  const floor = Number(row.values.floor);
  if (!Number.isFinite(floor)) throw new Error(`Row ${row.rowNumber}: Invalid floor "${row.values.floor}".`);
  const capacity = Number(row.values.capacity);
  if (!Number.isFinite(capacity) || capacity <= 0) throw new Error(`Row ${row.rowNumber}: Invalid capacity "${row.values.capacity}".`);
  const rent = Number(row.values.monthlyRent);
  if (!Number.isFinite(rent) || rent < 0) throw new Error(`Row ${row.rowNumber}: Invalid monthly rent "${row.values.monthlyRent}".`);

  const type = row.values.type.trim().toLowerCase();
  if (!VALID_TYPES.has(type)) {
    throw new Error(`Row ${row.rowNumber}: Room type must be single, double, or triple.`);
  }
  const status = row.values.status.trim().toLowerCase();
  if (!VALID_STATUS.has(status)) {
    throw new Error(`Row ${row.rowNumber}: Status must be available, occupied, or maintenance.`);
  }

  return {
    id: crypto.randomUUID(),
    roomNumber: row.values.roomNumber.trim(),
    block: row.values.block.trim(),
    floor,
    capacity,
    occupants: [],
    type: type as HostelRoom['type'],
    status: status as HostelRoom['status'],
    monthlyRent: rent,
  };
};

const hydrateRooms = async (rows: CsvRow[]) => {
  const valid: HostelRoom[] = [];
  const errors: string[] = [];
  for (let i = 0; i < rows.length; i += 1) {
    try {
      valid.push(buildHostelRoom(rows[i]));
    } catch (error) {
...
export default function HostelBulkUpload() {
  const { addHostelRoomsBulk } = useData();
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const { valid, errors } = await hydrateRooms(parsed);
      if (valid.length === 0) {
        setReport({ fileName: file.name, processed: parsed.length, added: 0, duplicates: [], errors });
        toast({ title: 'No rooms added', description: 'Every row in the file contains validation errors.', variant: 'destructive' });
        return;
      }
      const { inserted, duplicates } = addHostelRoomsBulk(valid);
      setReport({ fileName: file.name, processed: parsed.length, added: inserted.length, duplicates, errors });
      toast({
        title: `${inserted.length} room${inserted.length === 1 ? '' : 's'} added`,
  ...
  const downloadTemplate = useCallback(() => {
    ...
    link.href = url;
    link.download = 'campus-harmony-hostel-template.csv';
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Bulk Hostel Room Import</CardTitle>
        <CardDescription>CSV must include only these columns: {EXPECTED_COLUMNS_TEXT}. Status and type are lowercase keywords.</CardDescription>
      </CardHeader>
                ...
                <ul className="space-y-2 text-xs">
                  ...
                  {report.duplicates.map(id => (
                    <li key={id} className="flex items-start gap-2 text-amber-500">
                      <AlertTriangle className="mt-0.5 h-3.5 w-3.5" />
                      <span>Skipped duplicate room: {id}</span>
                    </li>
                  ))}
                </ul>
              </ScrollArea>
